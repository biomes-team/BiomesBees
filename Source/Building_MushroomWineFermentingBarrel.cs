...
public class Building_HoneyFermentingBarrel : Building, IThingHolder
{
	...
	public Building_HoneyFermentingBarrel()
	{
	...
	public void AddWort(Thing wort)
	{
		...
		if (num > 0)
		{
			...
			{
				Log.Warning(
					"Tried to add honeycomb to a barrel full of honey. Colonists should take out the finished honey first.");
				return;
			}
	...
	public override string GetInspectString()
	{
		...
		if (!Empty && !comp.Ruined)
		{
			if (Fermented)
			{
				stringBuilder.AppendLine("BMT_ContainsHoney".Translate(wortCount, 25));
			}
			else
			{
				stringBuilder.AppendLine("BMT_ContainsHoneyComb".Translate(wortCount, 25));
			}
		}
	...
	public Thing TakeOutBeer()
	{
		if (!Fermented)
		{
			Log.Warning("Tried to get honey but it's not yet fermented.");
			return null;
		}
